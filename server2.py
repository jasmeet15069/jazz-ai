"""
JAZZ Uncensored AI — v5.2 (Agent Jobs Edition — Fixed)
=======================================================
✅ All v5.1 features (SQLite, ChromaDB, RAG, Memory, Vision, Streaming)
...
v5.2 Fixes:
- DB_PATH always absolute (thread-safe sqlite3 access)
- asyncio.create_task crash callback — silent failures now logged
- _execute_job_sync writes log entry BEFORE running agent (visible immediately)
- /agent-jobs/{id}/run-debug endpoint for live error inspection
- Playwright import check at startup with friendly warning
- Error tracebacks captured and stored in agent_job_logs.error field
- time_window bypass on manual "Run Now" trigger
- Job status reset to idle on startup if stuck in "running"
"""
...
# FIX: Always use absolute path so thread-pool sqlite3 calls resolve correctly
...
# FIX: Check Playwright availability at startup
...
async def init_db() -> None:
    ...
    # FIX: Reset any jobs stuck in "running" from a previous crashed server
    await db_execute(
...
def _execute_job_sync(job_id:str, bypass_time_window:bool=False)->None:
    """
    FIX: Runs in a ThreadPoolExecutor thread.
    - Uses absolute DB_PATH (safe for threads)
    - Writes log row BEFORE running agent (visible in UI immediately)
    - Captures full traceback in error field
    - bypass_time_window=True skips window check (used by manual Run Now)
    """
    import sqlite3 as _s3
    ...
    def _dbw_output(log_id:str, output:str):
        """Update log output in real-time friendly way."""
        with _s3.connect(DB_PATH) as c:
            c.execute("UPDATE agent_job_logs SET output=? WHERE id=?",(output,log_id)); c.commit()

    row=_db("SELECT * FROM agent_jobs WHERE id=?",(job_id,))
    if not row:
        logger.error(f"[JOB] Job {job_id} not found in DB"); return
    job=dict(row)

    # Time window check (skipped for manual triggers)
    if not bypass_time_window:
        ...
        if not _is_within_time_window(tw_start,tw_end):
            logger.info(f"[JOB] '{job['name']}' outside window {tw_start}–{tw_end}, skipping tick")
            return

    now_iso=datetime.utcnow().isoformat()
    log_id=str(uuid.uuid4())

    # FIX: Write log row immediately so UI shows "running" straight away
    ...
    _dbw(
        ...
        (log_id, job_id, job["user_id"], "running",
         f"[{datetime.now().strftime('%H:%M:%S')}] ⚡ Job '{job['name']}' starting (type={job['job_type']})\n",
         now_iso)
    )
    ...
    if not _PLAYWRIGHT_AVAILABLE:
        jlog.log("❌ CRITICAL: Playwright is not installed on this server.")
        jlog.log("   Fix: pip install playwright && playwright install chromium")
        jlog.log("   Then restart the server and try again.")

    t0=time.time(); applied:List[Dict]=[]; error=None; final_status="completed"
...
async def lifespan(app:FastAPI):
    logger.info("="*60)
    logger.info("[STARTUP] JAZZ AI v5.2 starting (Agent Jobs Fixed Edition)...")
    ...
    logger.info(f"  PLAYWRIGHT: {'✅' if _PLAYWRIGHT_AVAILABLE else '❌  — pip install playwright && playwright install chromium'}")
...
app=FastAPI(
    title="JAZZ AI",version="5.2.0",
    description="Production AI platform with Agent Jobs (Fixed Edition)",
...
async def add_request_id(request:Request,call_next):
    ...
    response.headers["X-Request-ID"]=rid; response.headers["X-Powered-By"]="JAZZ-AI-v5.2"
    return response
...
if _static_dir.exists():
    ...
    @app.get("/")
    async def root(): return {"status":"JAZZ AI API","version":"5.2.0","docs":"/api/docs"}
...
async def run_agent_job_now(job_id:str,user:Dict=Depends(get_current_user)):
    """
    Trigger a job immediately, bypassing the time window check.
    FIX: added done_callback to catch and log silent task failures.
    """
    existing=await db_fetchone("SELECT id FROM agent_jobs WHERE id=? AND user_id=?",(job_id,user["sub"]))
    if not existing: raise HTTPException(404,"Job not found")

    # FIX: bypass_time_window=True so manual Run Now always executes
    task = asyncio.create_task(execute_job(job_id, bypass_time_window=True))

    # FIX: log silent crashes via done callback
    def _on_done(t:asyncio.Task):
        if t.cancelled():
            logger.error(f"[JOB] Task CANCELLED for job {job_id}")
        elif t.exception():
            logger.error(f"[JOB] Task CRASHED for job {job_id}: {t.exception()}", exc_info=t.exception())
...
async def run_agent_job_debug(job_id:str,user:Dict=Depends(get_current_user)):
    """
    NEW: Runs job synchronously in the request, returns full output + any error.
    Use this to diagnose why a job is not producing logs.
    """
    existing=await db_fetchone("SELECT id FROM agent_jobs WHERE id=? AND user_id=?",(job_id,user["sub"]))
    if not existing: raise HTTPException(404,"Job not found")
    ...
    try:
        ...
        tb=traceback.format_exc()
        return {"success":False,"error":str(exc),"traceback":tb,"playwright_available":_PLAYWRIGHT_AVAILABLE}
...
async def admin_stats(user:Dict=Depends(require_admin)):
    ...
    return {
        ...
        "playwright_available":_PLAYWRIGHT_AVAILABLE,
        "system_status":"healthy","version":"5.2.0","uptime_since":_SERVER_START.isoformat(),
    }
...
async def health():
    ...
    return {
        ...
        "services":{
            "sqlite":db_ok,"chromadb":chroma_client is not None,
            "mysql":_mysql_pool is not None,"playwright":_PLAYWRIGHT_AVAILABLE
        },
        "agents":[a.name for a in AGENTS],"version":"5.2.0",
...
if __name__=="__main__":
    ...
    uvicorn.run(
        ...
        access_log=True,timeout_keep_alive=75
    )
